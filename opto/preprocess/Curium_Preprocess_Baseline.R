...
# filter variables
###################
sess_opt <- c(-1,0) # -1:0 are baselines and 3:4 are post (followup)
...
idx2 <- idx %>% 
  ...
  table.express::summarise(preCount = sum(total_counts),
                           preTrials = sum(total_trials)) %>%
  table.express::mutate(preProp = preCount / preTrials) # proportion using sums across trials NOT average of session-specific proportions
# 
