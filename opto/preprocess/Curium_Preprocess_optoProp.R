...
# filter variables
###################
sess_opt <- c(1,2) # -1:0 are baselines and 3:4 are post (followup)
...
idx2 <- idx %>% 
  ...
  table.express::summarise(prop = mean(available),
                           total_counts = sum(available),
                           total_trials = .N) %>% # calculate proportions of being in right pose (available is indicator that met threshold of confidence)
  # sum over two baseline sessions
  table.express::group_by(mouse_id, target) %>%
  # total trials and target pose counts summed across both baseline sessions
  table.express::summarise(optoCount = sum(total_counts),
                           optoTrials = sum(total_trials)) %>%
  table.express::mutate(optoProp = optoCount / optoTrials) # proportion using sums across trials NOT average of session-specific proportions
# 
#   tidyr::pivot_wider(names_from = session_number, values_from = c(total_counts, total_trials) ) %>% # pivot wider to make columns
# colnames(idx2)[4:5] <- paste0("Sess_", sess_opt)
# colnames(idx2)[6:7] <- paste0("SessTrials_", sess_opt)
# setDF(idx2)
...
# idx2$preProp <- rowSums(idx2[,c("Sess_0", "Sess_-1")]) / rowSums(idx2[,c("SessTrials_0", "SessTrials_-1")])
...
# 
# #########################################
# # repeat the same but with total trials
# #########################################
# 
# idx2 <- idx %>% 
#   table.express::group_by(mouse_id, target, session_number) %>%
#   table.express::summarise( total_trials = n() ) %>% # calculate proportions of being in right pose (available is indicator that met threshold of confidence)
#   tidyr::pivot_wider(names_from = session_number, values_from = total_trials ) # pivot wider to make columns
