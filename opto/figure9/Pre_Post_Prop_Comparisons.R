...
fig <-
  ...
  geom_hline(yintercept=0, 
             ...
             size = rel(0.5),
             alpha = 0.7) + #
  ylab("Target Count Difference: Opto - Baseline Session" )+ 
  xlab("Group") + 
  ...
  theme( plot.title = element_text(hjust = 0.5, color="black", size=rel(1), face="bold"),
         ...
         axis.title = element_text(face="bold", color="black", size=rel(0.9)),
...
dat %>%
  dplyr::mutate(optoDiff = optoProp - preProp,
                Opto = ifelse(opsin == 1, "Opto", "Ctrl")) %>%
  dplyr::filter(target != -5) %>%
