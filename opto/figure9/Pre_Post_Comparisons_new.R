...
animals_incl <- c("1528", "1529", "1544", "1546", "15822", "15823", "15847",                
                  "239", "240", "241","242", "3440", "3441", "3442", "410", 
                  "dls-ai32jr-1", "dls-ai32jr-2", "dls-ai32jr-3", 
                  "dls-ai32jr-4", "dls-ai32jr-5")
...
# active session
dat <- data.table::fread(paste0(wd, "optoDA_optoProp.csv")) %>% # does not use feedback_status criteria for Available (to be consistent with pre- data)
  table.express::mutate(opsin = 1*I(!mouse_id %in% control_ids)) %>% # treatment group is 1
  table.express::select(mouse_id, target, optoCount, opsin) %>% 
  as_tibble()

dat <- dplyr::left_join(dat, baseline, by = c("mouse_id", "target"))

targets_msm <- data.frame(target = c(20, 27, 76, 17, 59, 30), new = 1:6) # order in MSM experiments
...
fig <-
  ...
  # relabel targets to be consistent with MSM ordering
  dplyr::left_join(targets_msm, by = "target") %>%
  dplyr::mutate(old_target_labels = target,
                target = new) %>%
                #mouse_id %in% c(animals_incl, control_ids)) %>%
  ...
  geom_hline(yintercept=0, 
             ...
             size = rel(0.5),
             alpha = 0.7) + #
  ylab(TeX("Opto - Baseline Session Difference: $\\sum_{t=1}^T Y_t - \\sum_{t=1}^{T_0} Y_t^0$")) + 
  xlab("Group") + 
  scale_fill_manual(values = myColors) +
  scale_color_manual(values = myColors ) +
  theme_classic(base_size = 12) +
  theme( plot.title = element_text(hjust = 0.5, color="black", size=rel(1), face="bold"),
         axis.text=element_text(face="bold",color="black", size=rel(0.9)),
...
dat %>%
  dplyr::mutate(optoDiff = optoCount - preCount,
                Opto = ifelse(opsin == 1, "Opto", "Ctrl")) %>%
  dplyr::filter(target != -5) %>%
  # dplyr::filter(mouse_id %in% c(animals_incl, control_ids)) %>%
  dplyr::group_by(mouse_id) %>%
  dplyr::summarise(mean_prop = mean(optoDiff),
                   Opto = unique(Opto)) %>%
  ggplot(aes( y = mean_prop, x = factor(Opto))) +
  geom_boxplot(aes(color = factor(Opto))) + 
  # geom_point(aes(color = factor(coef_idx))) + 
  geom_hline(yintercept=0, 
             linetype="dashed", 
             color = "black", 
...
dat$mouse_id <- as.numeric(as.factor(dat$mouse_id)) # recode IDs to be numeric

for(t in 1:length(targets)){
  # glm
  mod <- geepack::geeglm(Y ~ opsin * prePost, 
                    data = data.frame(dat[dat$target == targets[t],]), 
                    id = mouse_id,
                    family = poisson) 
  mod_summary <- summary(mod)
  betaMat[t,] <- coef(mod)
  seMat[t,] <- mod_summary$coefficients[,2]# as.numeric(sqrt(diag(sandwich::vcovCL(mod, type = "HC0", cluster = ~ mouse_id)))) # sandwich estimator
}
