# Effect Dissipation: Interaction with trt/control
# HR-MSM for optoDA
...
if(curium){
  ...
  code_wd <- "/Users/loewingergc/Desktop/NIMH Research/Causal/spontaneous_behavior_opto/Figures/Final Figures/dose_treatment_dissipate_3/code/"
  save_wd <- "/Users/loewingergc/Desktop/NIMH Research/Causal/spontaneous_behavior_opto/optoDA_results/Final/dose_treatment_dissipate_3/"
  # learners
...
# treatment group as effect modifier
dat <- data.table::fread(paste0(wd, "optoDA_data.csv")) %>%
            table.express::mutate(area = 1*I(mouse_id %in% control_ids)) %>% # treatment is baseline
            as_tibble()
...
targets <- targets[targets != -5] # fake target
...
for(cond in c("condit", "uncondit")){ # 
  ...
  for(lrn in c(NA)){
    ...
    for(Y_nm in c("Y1", "Y5", "Y10")){ #  
      ...
      # sum models
      coef_dim <- 8 # number of coefs for full factorial model
      ...
      for(tt in 1:length(targets)){
        ...
        if(cond == "uncondit"){
          ...
          mod <- msm_HR_optoIPW_DT(data = d[rows_incl,],
                                ...
                                W_name = "weights",
                                id_name = "id",
                                # remove_ind = c(4, 7:8, 12:16, 20, 23:32), 
                                trial_name = "trial",
