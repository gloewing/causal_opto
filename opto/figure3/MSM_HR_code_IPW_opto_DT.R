...
msm_HR_optoIPW_DT <- function(data,
  ...
  indicator_matrix <- rbind(0, diag(delta)) #expand.grid( rep(list(0:1), delta))
  ...
  setDF(data)
  # how many trials ago there was a stim
  dSum <- apply(data[,mycol], 1, function(x) which(x == 1)) # sets d_3 to 0
  dSum <- lapply(dSum, function(x) if(length(x) == 0){ return(0)} else{return(x)})
  dSum <- do.call(rbind, dSum)
  data$d_sum <- dSum # sets d_3 to 3
  ...
  if(family == "gaussian"){
    mod2 <- stats::lm(YY ~ d_sum * area,
                      weights = ww,
                      data = data)
  }else{
    data$YY <- as.integer(data$YY) # ensure converted
    
    mod2 <- stats::glm(YY ~ d_sum * area,
                       family = family,
                       weights = ww,
                       data = data)
  }
