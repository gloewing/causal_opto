...
msm_HR_optoIPW <- function(data,
  ...
  data[, keyby = .(id, session, regime_number), c(mycol) := (shift(I1, n = 0:(delta-1), type = "lag", fill = 0))] #[,regime_prod1 := Reduce(`*`, .SD), .SDcols = idx_regI1] #[,regime_prod1]
  ...
  data[, keyby = .(id, session, regime_number), c(mycol) := (shift(I0, n = 0:(delta-1), type = "lag", fill = 0))] #[,regime_prod1 := Reduce(`*`, .SD), .SDcols = idx_regI1] #[,regime_prod1]
  for(jj in 1:q){
    idx <- which(data$regime_number == jj)
  ...
  data[, ww := weight * (regime_p0 + regime_p1 == delta)]
  regime_ind <- 1*(data$regime_p0 + data$regime_p1 == delta)
  data <- data[ww != 0,]
  data$YY <- data[,..Y_name]
  # setkey(data, ww)
  # data <- data[ww!=0]
